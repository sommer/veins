...
AnalogueModel* PhyLayer::initializeAntenna(ParameterMap& params){
	double initialX = params["directionX"].doubleValue();
	double initialY = params["directionY"].doubleValue();
	return new AntennaModel(initialX, initialY);
}
...
AnalogueModel* PhyLayer::initializeSimplePathlossModel(ParameterMap& params){
	...
	// search for optionally defined path loss at 1m
	it = params.find("L0");
	if(it != params.end()) {
		L0 = it->second.doubleValue();
	}
	// get carrierFrequency from config
	it = params.find("carrierFrequency");
	...
	return new SimplePathlossModel(L0, alpha, carrierFrequency, &move, useTorus, playgroundSize, coreDebug);
