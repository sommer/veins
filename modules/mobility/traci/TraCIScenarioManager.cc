...
double TraCIScenarioManager::commandDistanceRequest(Coord position1, Coord position2, bool returnDrivingDistance) {
	...
	TraCIBuffer buf = queryTraCI(CMD_DISTANCEREQUEST, TraCIBuffer() << static_cast<uint8_t>(POSITION_2D) << double(p1.getX()) << double(p1.getY()) << static_cast<uint8_t>(POSITION_2D) << double(p2.getX()) << double(p2.getY()) << static_cast<uint8_t>(returnDrivingDistance ? REQUEST_DRIVINGDIST : REQUEST_AIRDIST));
...
void TraCIScenarioManager::commandSetPolygonShape(std::string polyId, std::list<Coord> points) {
	...
	for (std::list<Coord>::const_iterator i = points.begin(); i != points.end(); ++i) {
		...
		buf << static_cast<double>(pos.getX()) << static_cast<double>(pos.getY());
	}
...
TraCIScenarioManager::TraCICoord TraCIScenarioManager::omnet2traci(Coord coord) const {
	return TraCICoord(coord.getX() + netbounds1.getX() - margin, (netbounds2.getY() - netbounds1.getY()) - (coord.getY() - netbounds1.getY()) + margin);
}
