...
//              me = par("me");
	...
	case 1:
	{
		...
		fillPositionFromCoord(position, coord);
		fillPositionFromCoord(node[me].true_pos, coord);
...
//	num_anchors = (int) Max(num_nodes * (double) par("anchor_frac"), nr_dims + 1);
	num_anchors = ROUND(num_nodes * (double) par("anchor_frac"));
// 	if (num_anchors < nr_dims + 1) {
// 		error("%d anchors is too little for %d dimensions.", num_anchors, nr_dims);
// 	}
 ...
 * Timer methods
 ******************************************************************************/
timer_info *PositifLayer::timer(int reps, int handler, void *arg, double period)
{
	assert(handler < MAX_TIMERS);
	setRepeatTimer(handler, period, reps);
	if (arg)
...
// 		memcpy(NULL, &index, sizeof(index));
