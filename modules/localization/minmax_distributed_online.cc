/* -*- mode:c++ -*- ********************************************************
 * file:        minmax_distributed_online.cc
 *
 * author:      Aline Baggio
 *
 ...
 * implements a minmax_distributed_online localization algorithm (with moving anchors, 
 * see omnetpp.ini file)
 **************************************************************************/
 
#include "minmax_distributed_online.h"
...
Define_Module_Like(minmax_distributed_online, BaseLocalization);
...
void minmax_distributed_online::initialize(int stage)
{
 ...
 * NODE_POSITION_MESSAGE (NODE_BROADCAST_MESSAGE in minmax_distributed_online.h)) is a reply to a broadcast packet that we
 * have send and just causes some output before it is deleted
 **/
//void minmax_distributed_online::handleLowerMsg( cMessage* msg )
void minmax_distributed_online::handleMsg( cMessage* msg )
{
    switch(msg->kind()){
...
int minmax_distributed_online::calc_irregularity(){
...
// anchor to the anchor list, only if the position of the anchor has not changed. However, in a minmax_distributed_online algorithm

void minmax_distributed_online::newAnchor(cMessage * msg) {
...
void minmax_distributed_online::newNeighbor(cMessage * msg) {
	LocPkt * m = dynamic_cast<LocPkt *>(msg);
	NodeInfo * node = new NodeInfo(m, getPosition());
...
void minmax_distributed_online::update_box_with_negative_info(struct bounding_box anchor_box){
...
void minmax_distributed_online::rebuild_box(struct bounding_box box){
...
int minmax_distributed_online::boxes_intersect(struct bounding_box box){
 ...
 * @sa sendBroadcast
 **/
void minmax_distributed_online::handleSelfMsg(cMessage *msg) {}
void minmax_distributed_online::handleTimer(unsigned int index) {
	LocPkt* pkt;
 ...
 * the network layer
 **/
void minmax_distributed_online::sendBroadcast(LocPkt *pkt)
{
...
void minmax_distributed_online::wind_up(){
...
void minmax_distributed_online::display_error_vector(){
...
void minmax_distributed_online::finish() 
{
    if (! isAnchor){
    	wind_up();
    ...
    EV << "\t\tEnding minmax_distributed_online localization...\n";	
}
