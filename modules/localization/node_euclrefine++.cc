...
class Node_EuclRefinePP:public PositifLayer {
	timer_info *bc_blast;
	bool send_all_data;
	cLinkedList targets;
	...
	bool i_am_a_twin;
      ...
      public:
	 Module_Class_Members(Node_EuclRefinePP, PositifLayer, 0)
...
Define_Module_Like(Node_EuclRefinePP, PositifLayer);



void Node_EuclRefinePP::init(void)
{
	algorithm = 5;		// Output on raw data line only.
	version = 7;
...
void Node_EuclRefinePP::handleStartMessage(cMessage * msg)
{
	comp_position = NULL;
	bc_blast = NULL;
	bc_position = NULL;
...
void Node_EuclRefinePP::handleTimer(timer_info * timer)
{
	switch (*timer) {
	case TIMER_SND_POS:
...
void Node_EuclRefinePP::handleMessage(cMessage * msg, bool newNeighbor)
{
	if (newNeighbor) {
...
void Node_EuclRefinePP::handleStopMessage(cMessage * msg)
{
...
void Node_EuclRefinePP::blast(void)
{
	...
	for (cLinkedListIterator iter(targets); !iter.end(); iter++) {
		...
		if (target->cnt > 0 || send_all_data) {
			...
			i++;
			target->cnt--;
			logprintf("DDDDDDDDDDDD %d %d\n", target->cnt,
				  send_all_data ? 1 : 0);
		} else
			logprintf("DDDDDDDDDDDD\n");
	}
	msg->addPar("num_targets") = i;

	logprintf("DDDDDSENDING %d TARGETS\n", i);
...
void Node_EuclRefinePP::get_data_from_blast(cMessage * msg)
{
	//int src = msg->par( "src" );
...
void Node_EuclRefinePP::average_distances()
{
	// All neigbours of me
	for (cLinkedListIterator iter(nbrs); !iter.end(); iter++) {
...
void Node_EuclRefinePP::print_stored_data()
{
	logprintf("ENV--");
...
void Node_EuclRefinePP::print_data_from_blast(cMessage * msg)
{
	int src = msg->par("src");
	int num_nbrs = msg->par("num_nbrs");
	int num_targets = msg->par("num_targets");
...
void Node_EuclRefinePP::recva(cMessage * msg)
{
...
void Node_EuclRefinePP::target_based_triangulation(void)
{
...
bool Node_EuclRefinePP::exists_link(cLinkedList * list, int from, int to)
{
...
link_info *Node_EuclRefinePP::get_link(cLinkedList * list, int from, int to)
{
	for (cLinkedListIterator iter(*list); !iter.end(); iter++) {
		link_info *link = (link_info *) iter();
		if (link->from_idx == from && link->to_idx == to)
...
FLOAT Node_EuclRefinePP::diagonal(FLOAT a, FLOAT b, FLOAT c, FLOAT d, FLOAT e,
				  int sign)
{
...
diagonal_result Node_EuclRefinePP::safe_diagonal(FLOAT a, FLOAT b, FLOAT c,
						 FLOAT d, FLOAT e)
{
...
/*diagonal_result Node_EuclRefinePP::safe_diagonalPP( FLOAT a, FLOAT b, FLOAT c, FLOAT d, FLOAT e )
{
  diagonal_result result;
...
FLOAT Node_EuclRefinePP::vote_list(cLinkedList * votes)
{
#define A1D1 1
#define A1D2 2
#define A2D1 3
...
FLOAT Node_EuclRefinePP::nbr_vote(link_info * target, int *hops, int *min_hops)
{
	//
	...
	*min_hops = 10000;
	...
	for (cLinkedListIterator iter2(nbrs); !iter2.end(); iter2++, n2count++) {
		...
		n1count = 0;
		for (cLinkedListIterator iter(nbrs); !iter.end();
		     iter++, n1count++) {
			...
			*d12 = safe_diagonal(n1->distance,
			...
			bool found = false;
			for (cLinkedListIterator iterv(votes); !iterv.end();
			     iterv++) {
			...
			if (!found) {
				...
				*hops =
				    MAX(*hops,
					MAX(ntarget1->hops, ntarget2->hops));
				*min_hops =
				    MIN(*min_hops,
	...
	for (cLinkedListIterator iterv(votes); !iterv.end(); iterv++) {
...
FLOAT Node_EuclRefinePP::common_nbr(link_info * target, int *hops,
				    int *min_hops)
{
	//
	// the target structure give the distance from some neighbor, _to_ a target.
	...
	for (cLinkedListIterator iter2(nbrs); !iter2.end(); iter2++) {
		...
		for (cLinkedListIterator iter(nbrs); !iter.end(); iter++) {
			...
			for (cLinkedListIterator citer(nnbrs); !citer.end();
			     citer++) {
				...
				if ((cn = get_link(&(nbrs), me, cn1->to_idx))) {
					...
					/*      if( var2<-0.5 ) {
					   if( fabs( dab.d1 - cn->distance ) < MERROR*dab.d1 )
					   sidecn=-1;
					   else if( fabs( dab.d2 - cn->distance ) < MERROR*dab.d1 )
					   sidecn=1;
					   else
					   continue;
					   } else { */
...
/*
void Node_EuclRefinePP::find_optimal( link_info *target, int *ret_n1, int *ret_n2, int *ret_cn )
{
  link_info *n1, *n2, *cn;
...
FLOAT Node_EuclRefinePP::common_nbr( link_info *target ) {
...
void Node_EuclRefinePP::update_neighbor(cMessage * msg)
{
	nghbor_info *neighbor = NULL;
	int src = msg->par("src");
	logprintf("position msg from %d\n", src);
	bool found = false;
...
void Node_EuclRefinePP::do_triangulation(void *arg)
{
	flops++;
...
void Node_EuclRefinePP::sendPosition(void *arg)
{
	...
	logprintf("REFINE: sending position maybe.\n");
	...
	logprintf("REFINE: sending position\n");
...
void Node_EuclRefinePP::update_rectangle(link_info * anchor)
{
...
bool Node_EuclRefinePP::inside_rectangle(Position pos)
{
	return true;
...
bool Node_EuclRefinePP::inside_neighbors_range(Position pos)
{
	for (cLinkedListIterator iter(neighbors); !iter.end(); iter++) {
		nghbor_info *neighbor = (nghbor_info *) iter();
...
bool Node_EuclRefinePP::twins(nghbor_info * a, nghbor_info * b)
{
