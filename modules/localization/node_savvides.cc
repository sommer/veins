...
class Node_Savvides:public PositifLayer {
	double accuracy;
...
void Node_Savvides::init(void)
{
	...
	version = 8;
	/* This is a hack. */
	if (me == 0 || me == 3 || me == 5 || me == 8)
		status = STATUS_ANCHOR;
}

// Dispatch this timer to the proper handler function.
void Node_Savvides::handleRepeatTimer(unsigned int index)
{
	Enter_Method_Silent();
	...
	case TIMER_DO_TRI:
		do_triangulation(NULL);
		break;
	}
}

void Node_Savvides::handleMessage(cMessage * msg, bool newNeighbor)
{
	if (newNeighbor)
...
void Node_Savvides::handleStartMessage(cMessage * msg)
{
...
// 	bc_anchor->cnt = 0;
// 	addTimer(bc_anchor);
	setRepeatTimer(TIMER_SND_ANC, 1, reps);
...
// 		resetTimer(bc_anchor);
		resetRepeatTimer(TIMER_SND_ANC);
		anchors.insert(ME);

// 		bc_position = timer(reps, TIMER_SND_POS);
		bc_pos_delay = 0;
// 		addTimer(bc_position);
		setRepeatTimer(TIMER_SND_POS, 1, reps);
	}
}

void Node_Savvides::handleStopMessage(cMessage * msg)
{
	// send out final position for statistics
	cMessage *returnMsg = new cMessage("DONE", MSG_DONE);

	// OLD: using 2nd phase now to compare weighted avg and Savvides
...
// 			resetTimer(bc_position);
			resetRepeatTimer(TIMER_SND_POS);
			seqno[MSG_POSITION]++;
		}
		break;
...
// 			}
			if (!timerExists(TIMER_DO_TRI)) {
				setRepeatTimer(TIMER_DO_TRI, 1, 1);
			}
// 			resetTimer(comp_position);
			resetRepeatTimer(TIMER_DO_TRI);
		}
		break;
...
double Node_Savvides::true_pos_triangulate(void)
{
...
// PP: Cannot use wait() with handleMessage()
...
bool Node_Savvides::new_anchor(cMessage * msg)
{
...
bool Node_Savvides::inside_rectangle(Position pos)
{
...
void Node_Savvides::update_neighbor(cMessage * msg)
{
...
void Node_Savvides::sendPosition(void *arg)
{
	// Maybe we're only interested in the initial estimate this time.
	if (!do_2nd_phase)
		return;
...
bool Node_Savvides::twins(nghbor_info * a, nghbor_info * b)
{
...
void Node_Savvides::sendAnchor(void *anc)
{
...
//
// Two algorithms are executed: the first is Savvides' initial estimation
...
//
void Node_Savvides::savvides(void)
{
...
// 				bc_position = timer(1, TIMER_SND_POS);
			if (!timerExists(TIMER_SND_POS)) {
				setRepeatTimer(TIMER_SND_POS, 1, 1);
