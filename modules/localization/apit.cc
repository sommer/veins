...
void apit::handleTimer(unsigned int index){
	switch(index){
...
double apit::compute_dist(double real_dist, double dist_variance){
...
void apit::newAnchor(cMessage * msg) {
...
void apit::newNeighbor(cMessage * msg) {
	LocPkt * m = dynamic_cast<LocPkt *>(msg);
	NodeInfo * node = new NodeInfo(m, getPosition());
...
void apit::handleMsg(cMessage* msg){
	LocPkt * m = dynamic_cast<LocPkt *>(msg);
			...
			for (int j = i + 1 ; j < no_of_anchors - 1; j++)
				for (int k = j + 1; k < no_of_anchors ; k++)
				{
					...
					for (int nl = 0; nl < no_of_neighbours && (inside == true) ; nl++){
						...
						{
							if (neighbour_t[nl].anchor_ptr[x].id == id1 || neighbour_t[nl].anchor_ptr[x].id == id2 ||
							neighbour_t[nl].anchor_ptr[x].id == id3)
								if (count == 3)
									break;
								else
								{
...
int * apit::hasAnchors(int nl,int id1,int id2,int id3){
	int count = 0;
	int *res = new int[3];
...
int apit::tri_test(Coord pos,Coord a1,Coord a2,Coord a3){
...
Coord apit::compute_position(vector <struct pit_struct> pit, int pit_cnt){
	...
	int rows =	floor(playgnd->getX() / grid_size );
	int columns =	floor(playgnd->getY() / grid_size );
...
int apit:: in_triangle(Coord c, Coord anchor1, Coord anchor2, Coord anchor3){
