...
void CsmaCA::txPacket(MacPacket * msg) {
	assert(msg);
...
int CsmaCA::mustUseCA(MacPacket * pkt) {
...
void CsmaCA::rxFrame(MacPacket * msg){
	assert(msg);
...
void CsmaCA::sendRts() {
	...
	header.nav = NAV_RTS(tx_msg->getBitLength());
...
void CsmaCA::receiveRts(MacPacket * msg) {
...
void CsmaCA::receiveCts(MacPacket * msg) {
	assert(msg->local_to != BROADCAST);
	Header *header = (Header*)msg->getData();
	if(msg->local_to == macid()) {
		reg_rx_overhead(msg);
...
void CsmaCA::receiveAck(MacPacket * msg) {
	assert(msg->local_to != BROADCAST);
	if(msg->local_to == macid()) {
		reg_rx_overhead(msg);
		if(proto_state != PROTO_STATE_WFACK
...
void CsmaCA::receiveData(MacPacket * msg) {
	if(msg->local_to == macid()) {
