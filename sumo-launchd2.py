...
def run_sumo(runpath, sumo_command, shlex, config_file_name, remote_port, seed, client_socket, unused_port_lock, keep_temp, tmpdir):
    """
    Actually run SUMO.
    """
    ...
    #Override temporary directory default
...
def handle_launch_configuration(sumo_command, shlex, launch_xml_string, client_socket, keep_temp, tmpdir):
    """
    Process launch configuration in launch_xml_string.
    """
    # temporary director is as set
    tempfile.tempdir = tmpdir
    ...
    try:    
        ...
        # run SUMO
        result_xml = run_sumo(runpath, sumo_command, shlex, config_file_name, remote_port, seed, client_socket, unused_port_lock, keep_temp, tmpdir)
...
def handle_connection(sumo_command, shlex, conn, addr, keep_temp, tmpdir):
    """
    ...
    try:
        ...
        handle_launch_configuration(sumo_command, shlex, data, conn, keep_temp, tmpdir)
...
def wait_for_connections(sumo_command, shlex, sumo_port, bind_address, do_daemonize, do_kill, pidfile, keep_temp, tmpdir):
    """
    ...
    try:
        while True:
            ...
            thread.start_new_thread(handle_connection, (sumo_command, shlex, conn, addr, keep_temp, tmpdir))
...
#Daniel
    parser.add_option("-f", "--temp-dir", dest="tmpdir", default=os.path.join(tempfile.gettempdir()), help="root for temporary directory [default: %default]", metavar="TMPDIR")
    ...
    wait_for_connections(options.command, options.shlex, options.port, options.bind, options.daemonize, options.kill, options.pidfile, options.keep_temp, options.tmpdir)
