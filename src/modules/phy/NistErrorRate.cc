...
double
NistErrorRate::getChunkSuccessRate (unsigned int datarate, enum Bandwidth bw, double snr, uint32_t nbits)
{

	//todo: rename snr in snr_mW
	//get mcs from datarate and bw
	enum PHY_MCS mcs = getMCS(datarate, bw);
	//bw in Hz
	unsigned int bandwidth = getBandwidth(bw);
