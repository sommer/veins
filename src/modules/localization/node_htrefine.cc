...
typedef struct {
	...
	int last_hop_idx;
	Position position;
	//timer_info *bc_timer; Obsolete with new sending policy
	bool flood;		// Whether to flood this anchor or not.
	int cnt;		// Number of times this anchor will be sent in a message.
} anchor_info;
...
class Node_HTRefine:public PositifLayer {
	double accuracy;
	...
	int calib_src;
	bool i_am_a_twin;
	int triangulation_wait_counter;
	...
	double true_pos_triangulate(void);
      ...
      public:
	 //Module_Class_Members(Node_HTRefine, PositifLayer, 0)
...
//Define_Module_Like(Node_HTRefine, PositifLayer);

void Node_HTRefine::init(void)
{
	algorithm = 0;		// Output on raw data line only.
	version = 11;
...
void Node_HTRefine::handleTimer(timer_info * timer)
{
	switch (*timer) {
	case TIMER_SND_POS:
...
void Node_HTRefine::handleMessage(cMessage * msg, bool newNeighbor)
{
	if (newNeighbor)
...
void Node_HTRefine::handleStartMessage(cMessage * msg)
{
...
void Node_HTRefine::handleStopMessage(cMessage * msg)
{
...
void Node_HTRefine::anchor(cMessage * msg)
{
	switch (msg->getKind()) {
	case MSG_ANCHOR:
		if (new_anchor(msg)) {
...
void Node_HTRefine::unknown(cMessage * msg)
{
	switch (msg->getKind()) {
	case MSG_ANCHOR:
		if (new_anchor(msg)) {
...
bool Node_HTRefine::new_anchor(cMessage * msg)
{
	...
	for (int i = 0; i < num_anchors; i++) {
		...
		if (!found) {
			...
			if ((anchor->hop_cnt) < old_anchor->hop_cnt) {	// Found it, but the new one has a shorter path.
				old_anchor->hop_cnt = anchor->hop_cnt;
				old_anchor->last_hop_dst =
...
void Node_HTRefine::update_rectangle(anchor_info * anchor)
{
...
bool Node_HTRefine::inside_rectangle(Position pos)
{
...
bool Node_HTRefine::inside_neighbors_range(Position pos)
{
	for (cLinkedList::Iterator iter(neighbors); !iter.end(); iter++) {
		nghbor_info *neighbor = (nghbor_info *) iter();
...
void Node_HTRefine::update_neighbor(cMessage * msg)
{
...
void Node_HTRefine::hop_based_triangulation(void)
{
	...
	if (n >= phase1_min_anchors) {
		...
		for (cLinkedList::Iterator iter(anchors); !iter.end(); iter++) {
			...
			switch (terrain) {
			case SIMPLE_R:
				range = anchor->hop_cnt * range;
				break;
			case SIMPLE_AVG:
				range = anchor->hop_cnt * range;
				break;
			case CALIB_HOPS:
				range = anchor->hop_cnt * avg_hop_dist;
				break;
			case CALIB_RSSI:
				range =
...
// 		wait(n * nr_dims * nr_dims * msec);
		...
		if (0 <= res && res <= range) {
			...
			logprintf
			    ("Rejected (%f, %f)(%f) because of bad residu: %f\n",
			     next[0], next[1], distance(position,
							node[me].true_pos) /
			     range, res);
		}
		...
		for (cLinkedList::Iterator iter(anchors); !iter.end(); iter++) {
			...
			if (used)
				logprintf
				    (" -->(%d:(%4.0f,%4.0f)@%2dh,%4.0fd,%4.0fr)<-- ",
				     anchor->idx, anchor->position[0],
				     anchor->position[1], anchor->hop_cnt,
				     range_list[j], distance(node[me].true_pos,
							     node[anchor->idx].
							     true_pos));
			else
				logprintf(" (%d:(%4.0f,%4.0f)@%2dh) ",
					  anchor->idx, anchor->position[0],
					  anchor->position[1], anchor->hop_cnt);
		}
		logprintf("\n");
...
void Node_HTRefine::calibrate(void)
{
	...
	// Maybe use phase1_anchors here as well?
	if (anchors.length() + 1 > nr_dims) {	// count me too
		...
		for (cLinkedList::Iterator iter(anchors); !iter.end(); iter++) {
			...
			if (anchor->idx != me) {
				dist_sum +=
				    distance(position, anchor->position);
		...
		logprintf("Hop size calibrated to %3.2f\n", avg_hop_dist);
	}
}


void Node_HTRefine::sendPosition(void *arg)
{
...
void Node_HTRefine::sendCalib(void *arg)
{
...
void Node_HTRefine::sendAnchor(void *anc)
{
