...
#define NUMERICAL_EPS (double)0.001
...
PositionVector::getOverlapWith(const PositionVector& poly, double zThreshold) const {
    ...
    for (const_iterator i = begin(); i != end() - 1; i++) {
        if (poly.around(*i)) {
            ...
            if (fabs(closest.z() - (*i).z()) < zThreshold) {
                result = std::max(result, poly.distance2D(*i));
            }
        }
    }
    ...
    for (const_iterator i = poly.begin(); i != poly.end() - 1; i++) {
        if (around(*i)) {
            ...
            if (fabs(closest.z() - (*i).z()) < zThreshold) {
                result = std::max(result, distance2D(*i));
            }
        }
    }
    return result;
...
PositionVector::splitAt(double where) const {
    ...
    if (where <= POSITION_EPS || where >= length() - POSITION_EPS) {
        EV_WARN << "Splitting vector close to end (pos: " << where << ", length: " << length() << ")" << endl;
    }
...
PositionVector::getMaxGrade() const {
    ...
    for (int i = 1; i < (int)size(); ++i) {
        ...
        result = std::max(result, (double)fabs((p1.z() - p2.z()) / p1.distanceTo2D(p2)));
    }
    return result;
}
