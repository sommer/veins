...
#define ASSERTRTREE assert // RTree uses ASSERTRTREE( condition )
#ifndef Min
  #define Min __min 
#endif //Min
#ifndef Max
...
public:
  ...
  private:
    ...
    void Push(Node* a_node, int a_branchIndex)
    {
      ...
      ++m_tos;
      ASSERTRTREERTREE(m_tos <= MAX_STACK);
    }
...
{
  ASSERTRTREE(MAXNODES > MINNODES);
  ASSERTRTREE(MINNODES > 0);


  // We only support machine word size simple data type eg. integer index or object pointer.
  // Since we are storing as union with non data branch
  ASSERTRTREE(sizeof(DATATYPE) == sizeof(void*) || sizeof(DATATYPE) == sizeof(int));
...
void RTREE_QUAL::Insert(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const DATATYPE& a_dataId)
{
  ...
  {
    ASSERTRTREE(a_min[index] <= a_max[index]);
  }
#endif //_DEBUG
...
{
  ASSERTRTREE(a_rect && a_node && a_newNode);
  ASSERTRTREE(a_level >= 0 && a_level <= a_node->m_level);
...
{
  ASSERTRTREE(a_rect && a_root);
  ASSERTRTREE(a_level >= 0 && a_level <= (*a_root)->m_level);
#ifdef _DEBUG
  for(int index=0; index < NUMDIMS; ++index)
  {
    ASSERTRTREE(a_rect->m_min[index] <= a_rect->m_max[index]);
  }
#endif //_DEBUG  
...
{
  ASSERTRTREE(a_node && (a_index >= 0) && (a_index < MAXNODES));
  ASSERTRTREE(a_node->m_count > 0);
...
void RTREE_QUAL::SplitNode(Node* a_node, Branch* a_branch, Node** a_newNode)
{
  ...
  *a_newNode = AllocNode();
  ...
  ASSERTRTREE((a_node->m_count + (*a_newNode)->m_count) == parVars->m_total);
}
...
ELEMTYPEREAL RTREE_QUAL::RectVolume(Rect* a_rect)
{
  ...
  ASSERTRTREE(volume >= (ELEMTYPEREAL)0);
...
void RTREE_QUAL::GetBranches(Node* a_node, Branch* a_branch, PartitionVars* a_parVars)
{
  ...
  ASSERTRTREE(a_node->m_count == MAXNODES);
...
void RTREE_QUAL::ChoosePartition(PartitionVars* a_parVars, int a_minFill)
{
  ...
  ASSERTRTREE((a_parVars->m_count[0] + a_parVars->m_count[1]) == a_parVars->m_total);
  ASSERTRTREE((a_parVars->m_count[0] >= a_parVars->m_minFill) &&
...
void RTREE_QUAL::LoadNodes(Node* a_nodeA, Node* a_nodeB, PartitionVars* a_parVars)
{
  ...
  for(int index=0; index < a_parVars->m_total; ++index)
  {
    ASSERTRTREE(a_parVars->m_partition[index] == 0 || a_parVars->m_partition[index] == 1);
...
{
  ASSERTRTREE(a_parVars);
  ASSERTRTREE(!a_parVars->m_taken[a_index]);
...
{
  ASSERTRTREE(a_rect && a_node && a_listNode);
  ASSERTRTREE(a_node->m_level >= 0);
