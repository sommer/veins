...
void PhyLayer80211p::initialize(int stage) {
	...
	if (stage == 0) {
		...
		throw cException("The header length of the 802.11p standard is 46bit, please change your omnetpp.ini accordingly by either setting it to 46bit or removing the entry");
		}
...
AnalogueModel* PhyLayer80211p::initializeBreakpointPathlossModel(ParameterMap& params) {
	...
	if ( it != params.end() ) // parameter alpha1 has been specified in config.xml
	{
		...
		{
	        // throw error
			throw cException("TestPhyLayer::createPathLossModel(): alpha can't be smaller than specified in \
	...
	if ( it != params.end() ) // parameter carrierFrequency has been specified in config.xml
	{
		...
		{
			// throw error
			throw cException("TestPhyLayer::createPathLossModel(): carrierFrequency can't be smaller than specified in \
	               ConnectionManager. Please adjust your config.xml file accordingly");
		}
	}
	...
	if(alpha1 ==-1 || alpha2==-1 || breakpointDistance==-1 || L01==-1 || L02==-1) {
		throw cException("Undefined parameters for breakpointPathlossModel. Please check your configuration.");
	}
...
AnalogueModel* PhyLayer80211p::initializeSimpleObstacleShadowing(ParameterMap& params){
	...
	if ( it != params.end() ) // parameter carrierFrequency has been specified in config.xml
	{
		...
		{
			// throw error
			throw cException("initializeSimpleObstacleShadowing(): carrierFrequency can't be smaller than specified in ConnectionManager. Please adjust your config.xml file accordingly");
		}
	}
	else // carrierFrequency has not been specified in config.xml
	{
	...
	if (!obstacleControlP) throw cException("initializeSimpleObstacleShadowing(): cannot find ObstacleControl module");
