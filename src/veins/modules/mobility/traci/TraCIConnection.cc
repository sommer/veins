...
TraCIConnection* TraCIConnection::connect(const char* host, int port) {
	...
	if (initsocketlibonce() != 0) throw cException("Could not init socketlib");
	...
	if (saddr.s_addr != static_cast<unsigned int>(-1)) {
		...
		throw cException("Invalid TraCI server address: %s", host);
		return 0;
	}
	...
	*socketPtr = ::socket(AF_INET, SOCK_STREAM, 0);
	if (*socketPtr < 0) throw cException("Could not create socket to connect to TraCI server");

	if (::connect(*socketPtr, (sockaddr const*) &address, sizeof(address)) < 0) {
		throw cException("Could not connect to TraCI server. Make sure it is running and not behind a firewall. Error message: %d: %s", sock_errno(), strerror(sock_errno()));
	}
...
TraCIBuffer TraCIConnection::query(uint8_t commandId, const TraCIBuffer& buf) {
	...
	if (result == RTYPE_NOTIMPLEMENTED) throw cException("TraCI server reported command 0x%2x not implemented (\"%s\"). Might need newer version.", commandId, description.c_str());
	if (result == RTYPE_ERR) throw cException("TraCI server reported error executing command 0x%2x (\"%s\").", commandId, description.c_str());
...
std::string TraCIConnection::receiveMessage() {
	if (!socketPtr) throw cException("Not connected to TraCI server");
	...
	uint32_t msgLength;
	{
		...
		while (bytesRead < sizeof(uint32_t)) {
			...
			if (receivedBytes > 0) {
				...
				throw cException("Connection to TraCI server closed unexpectedly. Check your server's log");
			} else {
				if (sock_errno() == EINTR) continue;
				if (sock_errno() == EAGAIN) continue;
				throw cException("Connection to TraCI server lost. Check your server's log. Error message: %d: %s", sock_errno(), strerror(sock_errno()));
			}
		}
