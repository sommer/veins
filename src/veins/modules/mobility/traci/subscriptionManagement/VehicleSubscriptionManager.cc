...
VehicleSubscriptionManager::VehicleSubscriptionManager(std::shared_ptr<TraCIConnection>& connection, std::shared_ptr<TraCICommandInterface>& commandInterface)
    : SubscriptionManagerBase(connection, commandInterface)
    , updatedVehicles()
    , disappearedVehicles()
{
    subscribeToVehicleIDList();
}
...
bool VehicleSubscriptionManager::update(TraCIBuffer& buffer)
{
    ...
    if (isSubscribed(responseObjectID)) {
        ...
        vehicle.speed = speed;
        vehicle.angle = angle;
        vehicle.id = responseObjectID;
        vehicle.typeID = typeID;
        vehicle.signals = signals;
...
std::vector<Vehicle> VehicleSubscriptionManager::getUpdated() const
{
    return updatedVehicles;
}

std::vector<std::string> VehicleSubscriptionManager::getDisappeared() const
{
    std::vector<std::string> output;
    std::copy(disappearedVehicles.begin(), disappearedVehicles.end(), std::back_inserter(output));
    return output;
}

void VehicleSubscriptionManager::subscribeToVehicleIDList() {
...
void VehicleSubscriptionManager::processVehicleIDList(std::list<std::string>& idList)
{
    ...
    // basically: subscribed - idList
    // --> result is a list of vehicles that need to be unsubscribed
    std::set_difference(subscribed.begin(), subscribed.end(), idList.begin(), idList.end(), std::inserter(disappearedVehicles, disappearedVehicles.begin()));
    for (auto id : disappearedVehicles) {
        if (isSubscribed(id)) {
...
void VehicleSubscriptionManager::clearAPI() {
    updatedVehicles.clear();
    disappearedVehicles.clear();
}
