...
bool SimulationSubscriptionManager::update(TraCIBuffer& buffer) {
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j) {
        ...
        if (variable1_resp == TraCIConstants::VAR_TELEPORT_STARTING_VEHICLES_IDS) {
            ...
            uint32_t count;
            buffer >> count;
            EV_DEBUG << "TraCI reports " << count
                            << " vehicles starting to teleport." << endl;
            for (uint32_t i = 0; i < count; ++i) {
                std::string idstring;
                buffer >> idstring;
                mStartedTeleporting.push_back(idstring);
            }
            ...
            for (uint32_t i = 0; i < count; ++i) {
                std::string idstring;
                buffer >> idstring;
                mStartedParking.push_back(idstring);
            }
            ...
            uint32_t count;
            buffer >> count;
            EV_DEBUG << "TraCI reports " << count << " vehicles ending to park."
                            << endl;
            for (uint32_t i = 0; i < count; ++i) {
                std::string idstring;
                buffer >> idstring;
...
std::list<std::string> SimulationSubscriptionManager::getStartedTeleporting() {
    std::list<std::string> temp = mStartedTeleporting;
    mStartedTeleporting.clear();
    return temp;
}

std::list<std::string> SimulationSubscriptionManager::getStartedParking() {
    std::list<std::string> temp = mStartedParking;
    mStartedParking.clear();
    return temp;
}

std::list<std::string> SimulationSubscriptionManager::getEndedParking() {
    std::list<std::string> temp = mEndedParking;
    mEndedParking.clear();
    return temp;
}
