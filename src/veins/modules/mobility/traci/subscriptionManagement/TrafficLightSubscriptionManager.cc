...
TrafficLightSubscriptionManager::TrafficLightSubscriptionManager(std::shared_ptr<TraCIConnection>& connection, std::shared_ptr<TraCICommandInterface>& commandInterface)
    : SubscriptionManagerBase(connection, commandInterface)
    , updatedTrafficLights()
{
}

void TrafficLightSubscriptionManager::subscribe(std::string id)
{
...
std::vector<TrafficLight> TrafficLightSubscriptionManager::getUpdated() const
{
...
bool TrafficLightSubscriptionManager::update(TraCIBuffer& buffer)
{
    ...
    updatedTrafficLights.push_back(trafficLight);

    return true;
}

void TrafficLightSubscriptionManager::clearAPI() {
    updatedTrafficLights.clear();
}
