...
void TraCIScenarioManager::initialize(int stage) {
	...
	trafficLightModuleDisplayString =
	            par("trafficLightModuleDisplayString").stdstringValue();
	trafficLightModuleIds.clear();
	std::istringstream filterstream(par("trafficLightFilter").stdstringValue());
	std::copy(std::istream_iterator<std::string>(filterstream),
	            std::istream_iterator<std::string>(),
	            std::back_inserter(trafficLightModuleIds));
...
void TraCIScenarioManager::init_traci() {
	...
	if (!trafficLightModuleType.empty() && !trafficLightModuleIds.empty()) {
	        ...
	        cModuleType* tlModuleType = cModuleType::get(
	                trafficLightModuleType.c_str());

	        // query traffic lights via TraCI
	        std::list<std::string> trafficLightIds =
	                getCommandInterface()->getTrafficlightIds();
	        size_t nrOfTrafficLights = trafficLightIds.size();
	        int cnt = 0;
	        for (std::list<std::string>::iterator i = trafficLightIds.begin();
	                i != trafficLightIds.end(); ++i) {
	            std::string tlId = *i;
	            ...
	            Coord position =
	                    getCommandInterface()->junction(tlId).getPosition();

	            cModule *module = tlModuleType->create(
	                    trafficLightModuleName.c_str(), parentmod,
	                    nrOfTrafficLights, cnt);
	            module->par("externalId") = tlId;
	            module->finalizeParameters();
	            module->getDisplayString().parse(
	                    trafficLightModuleDisplayString.c_str());
	            module->buildInside();
...
void TraCIScenarioManager::subscribeToTrafficLightVariables(std::string tlId) {
    ...
    TraCIBuffer buf = connection->query(CMD_SUBSCRIBE_TL_VARIABLE,
...
void TraCIScenarioManager::unsubscribeFromTrafficLightVariables(
        std::string tlId) {
...
void TraCIScenarioManager::processTrafficLightSubscription(std::string objectId,
        TraCIBuffer& buf) {
    cModule* tlIfSubmodule = trafficLights[objectId]->getSubmodule(
            "tlInterface");
    TraCITrafficLightInterface *tlIfModule =
            dynamic_cast<TraCITrafficLightInterface*>(tlIfSubmodule);
    if (!tlIfModule) {
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j) {
        ...
        if (isokay != RTYPE_OK) {
            ...
            if (isokay == RTYPE_NOTIMPLEMENTED) {
                error(
                        "TraCI server reported subscribing to 0x%2x not implemented (\"%s\"). Might need newer version.",
                        response_type, description.c_str());
            } else {
                error(
        ...
        case TL_CURRENT_PHASE:
            tlIfModule->setCurrentPhaseByNr(
                    buf.readTypeChecked<int32_t>(TYPE_INTEGER), false);
            break;

        case TL_CURRENT_PROGRAM:
            tlIfModule->setCurrentLogicById(
                    buf.readTypeChecked<std::string>(TYPE_STRING), false);
            break;

        case TL_NEXT_SWITCH:
            tlIfModule->setNextSwitch(
                    SimTime(buf.readTypeChecked<int32_t>(TYPE_INTEGER),
                            SIMTIME_MS), false);
            break;

        case TL_RED_YELLOW_GREEN_STATE:
            tlIfModule->setCurrentState(
                    buf.readTypeChecked<std::string>(TYPE_STRING), false);
            break;

        default:
            error(
                    "Received unhandled traffic light subscription result; type: 0x%02x",
                    response_type);
            break;
        }
    }
