...
TraCIColor TraCICommandInterface::Vehicle::getColor() const {
    return traci->genericGetColor(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_COLOR,
...
std::list<std::string> TraCICommandInterface::getTrafficlightIds() {
    return genericGetStringList(CMD_GET_TL_VARIABLE, "", ID_LIST,
            RESPONSE_GET_TL_VARIABLE);
}

std::string TraCICommandInterface::Trafficlight::getCurrentState() const {
    return traci->genericGetString(CMD_GET_TL_VARIABLE, trafficLightId,
            TL_RED_YELLOW_GREEN_STATE, RESPONSE_GET_TL_VARIABLE);
}

int32_t TraCICommandInterface::Trafficlight::getDefaultCurrentPhaseDuration() const {
    return traci->genericGetInt(CMD_GET_TL_VARIABLE, trafficLightId,
            TL_PHASE_DURATION, RESPONSE_GET_TL_VARIABLE);
}

std::list<std::string> TraCICommandInterface::Trafficlight::getControlledLanes() const {
    return traci->genericGetStringList(CMD_GET_TL_VARIABLE, trafficLightId,
            TL_CONTROLLED_LANES, RESPONSE_GET_TL_VARIABLE);
}
...
std::list<std::list<TraCITrafficLightLink> > TraCICommandInterface::Trafficlight::getControlledLinks() const {
    ...
    TraCIBuffer buf = connection->query(commandId,
...
int32_t TraCICommandInterface::Trafficlight::getCurrentPhaseIndex() const {
    return traci->genericGetInt(CMD_GET_TL_VARIABLE, trafficLightId,
            TL_CURRENT_PHASE, RESPONSE_GET_TL_VARIABLE);
}

std::string TraCICommandInterface::Trafficlight::getCurrentProgramID() const {
    return traci->genericGetString(CMD_GET_TL_VARIABLE, trafficLightId,
            TL_CURRENT_PROGRAM, RESPONSE_GET_TL_VARIABLE);
}
...
int32_t TraCICommandInterface::Trafficlight::getAssumedNextSwitchTime() const {
    return traci->genericGetInt(CMD_GET_TL_VARIABLE, trafficLightId,
            TL_NEXT_SWITCH, RESPONSE_GET_TL_VARIABLE);
}

void TraCICommandInterface::Trafficlight::setState(std::string state) {
    TraCIBuffer buf = connection->query(CMD_SET_TL_VARIABLE,
            TraCIBuffer() << static_cast<uint8_t>(TL_RED_YELLOW_GREEN_STATE)
                    << trafficLightId << static_cast<uint8_t>(TYPE_STRING)
                    << state);
    ASSERT(buf.eof());
}
...
void TraCICommandInterface::Trafficlight::setPhaseDuration(int32_t duration) {
    TraCIBuffer buf = connection->query(CMD_SET_TL_VARIABLE,
            TraCIBuffer() << static_cast<uint8_t>(TL_PHASE_DURATION)
                    << trafficLightId << static_cast<uint8_t>(TYPE_INTEGER)
                    << duration);
    ASSERT(buf.eof());
}


void TraCICommandInterface::Trafficlight::setProgramDefinition(
        TraCITrafficLightProgram::Logic logic, int32_t logicNr) {
    TraCIBuffer inbuf;
...
TraCIColor TraCICommandInterface::genericGetColor(uint8_t commandId,
        std::string objectId, uint8_t variableId, uint8_t responseId) {

    uint8_t resultTypeId = TYPE_COLOR;
    TraCIColor result(0, 0, 0, 0);
