...
/**
 * @brief
 * Creates and moves nodes controlled by libsumo.
 *
 * All nodes created thus must have a LibsumoMobility submodule.
 *
 ...
 *
 */
class VEINS_API VehicleManagerLibsumo : public cSimpleModule {
public:
    ...
    VehicleManagerLibsumo();
    ~VehicleManagerLibsumo() override;
    ...
    LibsumoCommandInterface* getCommandInterface() const
    {
        return commandIfc.get();
    }

    LibsumoConnection* getConnection() const
    {
...
protected:
    ...
    std::unique_ptr<LibsumoConnection> connection;
    std::unique_ptr<LibsumoCommandInterface> commandIfc;
    ...
    MyMessageRetriever* myMessageRetriever;
    ...
    void processSimSubscription(std::string objectId, const libsumo::TraCIResults& results);
    void processVehicleSubscription(std::string objectId, const libsumo::TraCIResults& results);
    ...
    void processTrafficLightSubscription(std::string objectId, const libsumo::TraCIResults& results);
    /**
...
class VEINS_API VehicleManagerLibsumoAccess {
public:
    VehicleManagerLibsumo* get()
    {
        return FindModule<VehicleManagerLibsumo*>::findGlobalModule();
    };
};
