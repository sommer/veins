...
void DemoBaseApplLayer::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        if (FindModule<LibsumoMobility*>::findSubModule(getParentModule())) {
            mobility = LibsumoMobilityAccess().get(getParentModule());
        ...
        findHost()->subscribe(LibsumoMobility::parkingStateChangedSignal, this);
...
void DemoBaseApplLayer::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj, cObject* details)
{
    ...
    else if (signalID == LibsumoMobility::parkingStateChangedSignal) {
