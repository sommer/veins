 ...
 * Thus, LibsumoMobility::updatePosition sets the BaseMobility's orientation and direction to the same value.
 ...
 * @ingroup mobility
 */
class VEINS_API LibsumoMobility : public BaseMobility {
public:
    ...
    {
        if (external_id == "") throw cRuntimeError("LibsumoMobility::getExternalId called with no external_id set yet");
        return external_id;
    }
    ...
    {
        if (road_id == "") throw cRuntimeError("LibsumoMobility::getRoadId called with no road_id set yet");
        return road_id;
    }
    virtual double getSpeed() const
    {
        if (speed == -1) throw cRuntimeError("LibsumoMobility::getSpeed called with no speed set yet");
        return speed;
    }
    virtual VehicleSignalSet getSignals() const
    {
        if (signals.test(VehicleSignal::undefined)) throw cRuntimeError("LibsumoMobility::getSignals called with no signals set yet");
        return signals;
    }
    /**
     * returns heading.
    ...
    {
        if (heading.isNan()) throw cRuntimeError("LibsumoMobility::getHeading called with no heading set yet");
        return heading;
    }
    virtual VehicleManagerLibsumo* getManager() const
    {
        if (!manager) manager = VehicleManagerLibsumoAccess().get();
        return manager;
    }
    virtual LibsumoCommandInterface* getCommandInterface() const
    {
        if (!commandInterface) commandInterface = getManager()->getCommandInterface();
        return commandInterface;
    }
    virtual LibsumoCommandInterface::Vehicle* getVehicleCommandInterface() const
    {
        if (!vehicleCommandInterface) vehicleCommandInterface = new LibsumoCommandInterface::Vehicle(getCommandInterface()->vehicle(getExternalId()));
...
protected:
    ...
    /**
     * Calling this method on pointers of type LibsumoMobility is deprecated in favor of calling either getHostSpeed or getSpeed.
     */
...
class VEINS_API LibsumoMobilityAccess {
public:
    LibsumoMobility* get(cModule* host)
    {
        LibsumoMobility* traci = FindModule<LibsumoMobility*>::findSubModule(host);
        ASSERT(traci);
        return traci;
    };
};
