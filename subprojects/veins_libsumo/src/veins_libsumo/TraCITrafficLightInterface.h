...
public:
    ...
    {
        if (!manager) {
            manager = VehicleManagerLibsumoAccess().get();
        }
        return manager;
    }
    ...
    virtual LibsumoCommandInterface::Trafficlight* getTlCommandInterface() const
    {
        if (!tlCommandInterface) {
            tlCommandInterface = new LibsumoCommandInterface::Trafficlight(getCommandInterface(), external_id);
        }
...
protected:
    ...
    /** reference to the simulations traffic light-specific TraCI command interface */
    mutable LibsumoCommandInterface::Trafficlight* tlCommandInterface;
