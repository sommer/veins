...
void DeciderTest::runTests()
{
	...
	runDeciderTests("SNRThresholdDecider");
...
void DeciderTest::executeTestCase(TestCaseIdentifier testCase) {
	...
	if(deciderName == "SNRThresholdDecider") {
		executeSNRTestCase();
	} else if(deciderName == "SNRThresholdDeciderNew") {
...
Decider* DeciderTest::initDeciderTest(std::string name, ParameterMap& params) {
	...
	if(name == "SNRThresholdDecider") {
		...
		return new TestSNRThresholdDecider(this, snrThreshold, sensitivity, myIndex, coreDebug);
	} else if(name == "SNRThresholdDeciderNew") {
		// parameters for original TestDecider (tests/basePhyLayer with testBaseDecider = true)
		double snrThreshold = 10.0;
...
//
void DeciderTest::executeSNRTestCase()
{
	ChannelState cs;
