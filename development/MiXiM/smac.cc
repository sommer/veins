...
Define_Module_Like( SMac, MacClass );

void SMac::initialize() {
	Mac::initialize();
	printfNoInfo(PRINT_INIT, "\t\tSMAC initializing...");
...
void SMac::finish() {
	Mac::finish();
	printfNoInfo(PRINT_INIT, "\t\tSMac ending...");
}

void SMac::txPacket(Packet * msg) {
	assert(msg);
...
void SMac::evalState() {
	if (proto_state == PROTO_STATE_IDLE && !isReceiving()) {
		...
		if (nav_state == NAV_STATE_CLEAR && sched_state != SCHED_STATE_SLEEP) {
			...
			if (must_send_sync) {
				printf(PRINT_MAC, "preparing to send SYNC %s",tx_msg?"(data pending)":"");
				// start contending
...
void SMac::rxFrame(Packet * msg) {
	...
	if (proto_state == PROTO_STATE_WFCTS && (PKT_KIND(msg) != KIND_CTS || msg->local_to != node->getNodeId())) {
	...
	if (proto_state == PROTO_STATE_WFACK && (PKT_KIND(msg) != KIND_ACK || msg->local_to != node->getNodeId())) {
...
void SMac::receiveSync(Packet * msg) {
	...
	// check offset
	int ftime = (int)node->getCurrentTime() + stime;	// frame start time
...
void SMac::receiveRts(Packet * msg) {
...
void SMac::receiveCts(Packet * msg) {
	assert(msg->local_to != BROADCAST);
	if (msg->local_to == node->getNodeId()) {
		reg_rx_overhead(msg);
...
void SMac::receiveAck(Packet * msg) {
	assert(msg->local_to != BROADCAST);
	if (msg->local_to == node->getNodeId()) {
		reg_rx_overhead(msg);
...
void SMac::receiveData(Packet * msg) {
	if (msg->local_to == node->getNodeId()) {
...
void SMac::calcSchedState() {
	...
	for (i = 0; i < sched_count; i++) {
		if (in_f > sched_table[i] && (unsigned)in_f < sched_table[i] + listen_time) {
			...
			assert(left <= listen_time + 5);
			printf(PRINT_MAC, "calc_sched: in foreign frame %d, left = %u", i, (unsigned) left);
			setSchedTimeout(left);
			return;
		}
	}
