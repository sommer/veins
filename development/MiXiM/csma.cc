...
Define_Module_Like( Csma, MacClass );
...
void Csma::gotoTransmit() {
	assert(tx_msg);
	Packet * msg = (Packet *)tx_msg->dup(); // delete tx msg itself later
	
	setRadioTransmit();

	msg->setLocalFrom(node->getNodeId());
