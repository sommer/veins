...
class GMac: public Mac {
	...
	Module_Class_Members(GMac, Mac, 0);
...
protected:
	...
	/* Packet that will be sent to the radio. */
	Packet *packet;
	/* Packet that is received (and will be sent back to) the routing layer. */
	Packet *tx_msg;
...
public:
	...
	virtual int firstToWake(vector<int> *nodes);
};
