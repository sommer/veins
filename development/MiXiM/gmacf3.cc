...
GMac::SlotState GMacF3::getCurrentSlotState() {
	...
	if (current_slot >= slots)
		return tx_msg && (tx_msg->local_to % slots) == (node->getNodeId() % slots) &&
			(!slotted_bcast || node->getNodeId() % bcast_slots == current_slot - slots) ? SSTATE_SEND_RECEIVE : SSTATE_RECEIVE;
