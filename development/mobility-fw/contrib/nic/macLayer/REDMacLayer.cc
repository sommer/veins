...

{
    ...
    if(stage==0){
      ...
      channel = hasPar("defaultChannel") ? par("defaultChannel") : 0;
     	...
     	radio->setBitrate(23405 * 8 / 10); // UART  1 byte ->  10 bit, but we work with bytes only and use a smaller bitrate instead
      ...
      splitControlStart();
    	...
    	// physical header: 6 bytes + RedMac: 12 bytes + 2 bytes 4b6b codiert
    	// to do: adjust snreval headerlength to 6*8 omnetpp.ini
    	headerLength = (int)(14.0 * 1.5 * 8.0); 
   }
}
            ...
            if(error == SUCCESS) {
              ...
              if(error == SUCCESS) {
                if(ackIsForMe(pkt)) {
                    ...
                    cInfo->setStrength(milliToDB(rssi)); //TODO: dB umrechnen
...
void REDMacLayer::signalSendDone(Error_t error){
     ...
     longRetryCounter = 0;
     shortRetryCounter = 0;
     //converts a milli watt value into a dBm value
     cInfo->setStrength(milliToDB(rssi)); 
     pkt->setControlInfo(cInfo);
     delete cInfo;
