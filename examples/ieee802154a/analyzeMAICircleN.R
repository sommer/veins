...
loadScaData <-function() {
  fileList <- sort(list.files(path="./akshadow/results", pattern="csv$"))
  res <- array(0, c(length(fileList), 6))
  info <- array(0, c(length(fileList), 3))
  i <- 1
  for (filename in fileList) {
    res[i, ] <- loadCSV(paste("./akshadow/results/", filename, sep=""))
    info[i, ] <- loadInfo(paste("./akshadow/results/", filename, sep=""))
    i <- i+1
  }
  list(v1=res, v2=info)
}
