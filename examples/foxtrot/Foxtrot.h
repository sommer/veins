...
#define FoxtrotPacket_SIZE(x) (offsetof(FoxtrotPacket,data)+(sizeof(foxtrot_data)*PT(x)))
...
class Foxtrot: public BaseAggLayer, public FrameTimer, public Timer
{
  ...
  protected:
	...
	{
		error("Foxtrot does not handle control messages");
	};
	...
	inline bool merge(uint8_t *subset, uint8_t *delete_count);
