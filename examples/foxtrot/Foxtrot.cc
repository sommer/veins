...
typedef enum
{ TIMER_SEND = 0, TIMER_PROCESS } FoxtrotTimer;
	...
	*delete_count = 0;
	...
	for (k = 0; k < count; k++)
	{
		...
		if (!timeset || storage[k]->getWhen() < midrange->getWhen())
		{
			...
			EV << "Setting midrange time using "<<k<<endl;
			timeset = true;
		}
	}
	for (k = count - 1; k >= 0; k--)
...
uint8_t Foxtrot::mergeableSubset(FoxtrotPacketStorage * list, uint8_t * subset)
{
	...
	for (i = 0; i < count; i++)
	{
		DBG("Data point (ms) %d: " FP_PRINTF " - " FP_PRINTF " (box " FP_PRINTF "-" FP_PRINTF "," FP_PRINTF "-" FP_PRINTF "). Time=%d\n", i, (*list)[i]->getData(0).min, (*list)[i]->getData(0).max, (*list)[i]->getLoc(X_DIR).min, (*list)[i]->getLoc(X_DIR).max, (*list)[i]->getLoc(Y_DIR).min,
			(*list)[i]->getLoc(Y_DIR).max, (*list)[i]->getWhen());
	}
