 ...
 * 
 * PathlossConstMapping is subclassed from SimpleConstMapping for convenience.
...
public:
	 ...
	 */
	PathlossConstMapping(const DimensionSet& dimensions,
						const Argument& min,
						const Argument& max,
						const Argument& interval,
						const double _attValue) :
		SimpleConstMapping(dimensions, min, max, interval),
		attValue(_attValue)
	{
...
public:
	 ...
	 * 
	 */
	PathlossModel(double alpha, double _carrierFrequency, const Move* _myMove,
					bool _useTorus, const Coord& _playgroundSize):
