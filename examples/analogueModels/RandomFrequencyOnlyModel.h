 ...
 * everytime we want to set a value of the Mapping we will just pass 0 as parameter
 * for the time dimension.
 * 
 * Note: Currently the multiply-method of the Mapping interface can't handle
 * two mappings where the second mapping has "holes" in its domain. (see multiply-method
 * for details). So this method whouldn't work with a space-only-mapping since the missing
 * frequency dimension whould produce a "hole" when multiplied with a space-freq-time-mapping.
 * This problem will probably be solved in the end. 
 */
