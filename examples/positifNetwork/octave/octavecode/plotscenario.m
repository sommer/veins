function plotscenario(data)
  ...
  for k=i:size(data,1)
    ...
    if(node1<node2)
      ...
      plotcmd=sprintf("%s plotdata using %i:%i w linespoints 8,", plotcmd, 2*j-1, 2*j);
      j=j+1;
    endif
  endfor
  ...
  for k=1:i-1
    if data(k,4)==1
      ...
      plotcmd=sprintf("%s plotdata using %i:%i w points 12,", plotcmd, 2*j-1, 2*j);
      j=j+1;
    endif
  endfor
