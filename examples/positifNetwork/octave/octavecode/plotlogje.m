function plotlogje(data,plottitle,areasize)
  ...
  if(nargin!=1 && nargin!=2 && nargin!=3)
    usage("plotlogje(data,[plottitle=logdata],[areasize=100])");
  endif
  if(nargin==1)
    areasize=100;
    plottitle="logdata";
  ...
  for i=1:size(data,1)
	...
	if data(i,6)==3 #BAD
	  ...
	  plotcmd=sprintf("%s plotdata using %i:%i w lines 8,", plotcmd, 4*i-1, 4*i);
	endif
      endif
	...
	plotcmd=sprintf("%s plotdata using %i:%i w points 12,", plotcmd, 4*i-3, 4*i-2);
      endif
      if data(i,6)==2 #UNKNOWN
	plotcmd=sprintf("%s plotdata using %i:%i w points 9,", plotcmd, 4*i-3, 4*i-2);
      endif
      if data(i,6)==3 #BAD
	plotcmd=sprintf("%s plotdata using %i:%i w points 8,", plotcmd, 4*i-3, 4*i-2);
      endif
